...
// Embedded server source files
//go:embed main.go
var mainGoSource string

//go:embed server.go
var serverGoSource string

//go:embed web.go
var webGoSource string

//go:embed languages.go
var languagesGoSource string

//go:embed go.mod
var goModSource string

//go:embed go.sum
var goSumSource string

//go:embed launcher_main.go
var launcherMainGoSource string

// Map of filename to source code for main server
var embeddedSources = map[string]string{
	"main.go":         mainGoSource,
	"server.go":       serverGoSource,
	"web.go":          webGoSource,
	"languages.go":    languagesGoSource,
	"dependencies.go": dependenciesGoSource,
	"go.mod":          goModSource,
	"go.sum":          goSumSource,
}

// Map of filename to source code for launcher
var embeddedLauncherSources = map[string]string{
	"launcher_main.go": launcherMainGoSource,
	"server.go":        serverGoSource,
	"web.go":           webGoSource,
	"languages.go":     languagesGoSource,
	"dependencies.go":  dependenciesGoSource,
...
// Note: dependencies.go is already compiled into this binary,
// but we need to include its source for the server build.
// We'll read it from the embedded copy below.
const dependenciesGoSource = `package main
